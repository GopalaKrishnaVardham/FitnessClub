...
public class FitnessClub {
    private static final int MAX_LESSONS_PER_DAY = 2;
    ...
    private static final double[] FITNESS_PRICES = {10.0, 25.0, 20.0, 15.0};
    private static String[] DAYS_OF_WEEK = {"Saturday", "Sunday"};
    private static double[] incomes = new double[FITNESS_TYPES.length];
    private static int[][] bookings = new int[NUM_DAYS][MAX_LESSONS_PER_DAY * NUM_DAYS];
    ...
    private static void bookLesson(Scanner scanner) {
        ...
        if(option == 1) {
            for (int i = 0; i < availableLessons.length; i++) {
                ...
                System.out.println((i + 1) + ". " + fitnessType + " on " + getDayOfWeek(day % 2) +
                        " at " + getTimeOfDay(lesson) + " with a price of $" + FITNESS_PRICES[lesson]);
            }
        }
        ...
        int day = lessonIndex % MAX_LESSONS_PER_DAY;
        int lesson = lessonIndex % FITNESS_TYPES.length;
        String fitnessType = FITNESS_TYPES[lesson];
        ...
        if (option == 1) {
            ...
            for (int i = 0; i < availableLessons.length; i++) {
                ...
                System.out.println((i + 1) + ". " + fitnessTypeA + " on " + getDayOfWeek(dayA % 2) +
                        " at " + getTimeOfDay(lessonA) + " with a price of $" + FITNESS_PRICES[lessonA]);
            }
            ...
            int newDay = newLessonIndex % MAX_LESSONS_PER_DAY;
            ...
            System.out.println("You have selected " + FITNESS_TYPES[newLesson] + " on " + getDayOfWeek(newDay%2) +
                    " at " + getTimeOfDay(newLesson) + " with a price of $" + FITNESS_PRICES[newLesson]);
            ...
            incomes[fitnessTypeIndex] -= FITNESS_PRICES[lesson];
    ...
    private static void attendLesson(Scanner scanner) {
        ...
        double income = FITNESS_PRICES[lesson];
    ...
    public static void generateMonthlyLessonReport() {
        ...
        for (int i = 0; i < NUM_DAYS; i++) {
            for (int j = 0; j < MAX_LESSONS_PER_DAY * NUM_DAYS; j++) {
                if (bookings[i][j] > 0 && review.getRating(j% FITNESS_TYPES.length) > 0) {
                    int fitnessTypeIndex = j % FITNESS_TYPES.length;
                    String fitnessType = FITNESS_TYPES[fitnessTypeIndex];
                    double income = FITNESS_PRICES[fitnessTypeIndex] * bookings[i][j];
    ...
    private static int[] getAvailableLessonsForDay(int day) {
        int[] availableLessons = new int[MAX_LESSONS_PER_DAY * NUM_DAYS];
        int index = 0;
        for (int i = 0; i < MAX_LESSONS_PER_DAY * NUM_DAYS; i++) {
    ...
    private static int[] parseBookingId(int bookingId) {
        ...
        if (!isValidBookingId(day, lesson, customerId)) {
    ...
    private static boolean isValidBookingId(int day, int lesson, int customerId) {
        return day >= 0 && day < NUM_DAYS &&
                lesson >= 0 && lesson < (NUM_DAYS * MAX_LESSONS_PER_DAY) &&
